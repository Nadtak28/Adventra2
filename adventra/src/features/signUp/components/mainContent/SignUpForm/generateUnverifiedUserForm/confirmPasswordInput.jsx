...
import { updateField } from "../../../../hook/generateUnverifiedUserSlice";

export default function ConfirmPasswordInput() {
  const dispatch = useDispatch();
  const { password_confirmation, validationErrors, submitted } = useSelector((state) => state.generateUnverifiedUser);
    ...
    <label className="flex flex-col min-w-40 flex-1">
      <div className="flex items-center bg-[#e8f2f1] rounded-xl h-14 px-4 focus-within:outline-none">
        ...
        <input
          ...
          onChange={(e) => dispatch(updateField({ field: "password_confirmation", value: e.target.value }))}
      ...
      {submitted && validationErrors.password_confirmation && (
