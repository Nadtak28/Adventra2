function EventInfoItem({ icon: IconComponent, label, value, arrow = true }) {
  return (
    <div className="flex items-center gap-4 bg-[#f8fbfb] px-4 min-h-14 justify-between">
      <div className="flex items-center gap-3 flex-1 min-w-0">
        {IconComponent && <IconComponent />}
        <p className="text-[#519489] text-base font-normal leading-normal truncate">{label}</p>
      </div>
      <div className="flex items-center justify-end gap-2">
        <span className="text-[#0e1a18] text-base font-normal leading-normal truncate">{value}</span>
        {arrow && (
